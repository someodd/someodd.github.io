...
[Open Stream](https://radio.someodd.zip/){:.button .primary}
...
Listen: http://radio.someodd.zip/

[View Source](https://github.com/someodd/whisper-radio)
...
Simply listen with the link below:

https://radio.someodd.zip/

You can listen to the live demo on [the official Whisper Radio
station](https://radio.someodd.zip/)! You can also open that link in
...
```
renew_hook = cat /etc/letsencrypt/live/radio.someodd.zip/fullchain.pem /etc/letsencrypt/live/radio.someodd.zip/privkey.pem > /etc/icecast2/bundle.pem && service icecast2 restart
```
...
server {
    ...
    location / {
        proxy_pass https://localhost:8443/stream;
...
Original content in gopherspace: [gopher://gopher.someodd.zip:70/0/phlog/whisper-radio.gopher.txt](gopher://gopher.someodd.zip:70/0/phlog/whisper-radio.gopher.txt)
